...
class AuthSpec extends AsyncFreeSpec with AsyncIOSpec with Matchers with UserFixture {
  ...
  val mockedTokens: Tokens[IO] = new Tokens[IO] {
    override def getToken(email: String): IO[Option[String]] =
      if (email == romanEmail) IO.pure(Some("abc123"))
      else IO.pure(None)
  ...
  def probedEmais(users: Ref[IO, Set[String]]): Emails[IO] = new Emails[IO] {
  ...
  "Auth 'algebra'" - {
    "login should return None if the user doesn't exist" in {
      val program = for {
        ...
        maybeToken <- auth.login("user@yandex.com", "password")
      } yield maybeToken
    ...
    "login should return None if the user exists but the password is wrong" in {
      val program = for {
        auth       <- LiveAuth[IO](mockedUsers, mockedTokens, mockedEmails)
        maybeToken <- auth.login(romanEmail, "wrongpassword")
      } yield maybeToken
    ...
    "login should return token if the user exists and the password is correct" in {
      val program = for {
        auth       <- LiveAuth[IO](mockedUsers, mockedTokens, mockedEmails)
        maybeToken <- auth.login(romanEmail, romanPassword)
      } yield maybeToken

      program.asserting(_ shouldBe defined)
    }

    "signing up should not create a user an existing email" in {
      val program = for {
        auth <- LiveAuth[IO](mockedUsers, mockedTokens, mockedEmails)
    ...
    "signing up should create a completely new user" in {
      ...
      program.asserting {
        case Some(user) =>
          user.email shouldBe "bob@yandex.com"
    ...
    "changePassword should return Right(None) if the user doesn't exist" in {
      val program = for {
        auth   <- LiveAuth[IO](mockedUsers, mockedTokens, mockedEmails)
        result <- auth.changePassword("popa@yandex.com", NewPasswordInfo("oldpw", "newpw"))
      } yield result

      program.asserting(_ shouldBe Right(None))
    }

    "changePassword should return Left if the password is incorrect" in {
      val program = for {
        auth   <- LiveAuth[IO](mockedUsers, mockedTokens, mockedEmails)
        result <- auth.changePassword(romanEmail, NewPasswordInfo("oldpw", "newpw"))
      } yield result
    ...
    "changePassword should correctly change password if all details are correct" in {
      val program = for {
        auth   <- LiveAuth[IO](mockedUsers, mockedTokens, mockedEmails)
        result <- auth.changePassword(romanEmail, NewPasswordInfo(romanPassword, "scala123"))
    ...
    "recoverPassword should fail for a user that does not exist, even if the token is correct" in {
      val program = for {
        auth    <- LiveAuth[IO](mockedUsers, mockedTokens, mockedEmails)
        result1 <- auth.recoverPasswordFromToken("someone@gmailcom", "abc123", "lala")
        result2 <- auth.recoverPasswordFromToken("someone@gmailcom", "wrongtoken", "lala")
    ...
    "recoverPassword should fail for a user that exists, but the token is wrong" in {
      val program = for {
        auth   <- LiveAuth[IO](mockedUsers, mockedTokens, mockedEmails)
        result <- auth.recoverPasswordFromToken(romanEmail, "wrongtoken", "h4ch3d")
      } yield result
    ...
    "recoverPassword should succeed for a correct combination of user/token" in {
      val program = for {
        auth   <- LiveAuth[IO](mockedUsers, mockedTokens, mockedEmails)
        result <- auth.recoverPasswordFromToken(romanEmail, "abc123", "rockstar")
      } yield result
    ...
    "sending recovery passwords should fail for a user that doesn't exist" in {
      val program = for {
        ...
        emails <- IO(probedEmais(set))
        auth   <- LiveAuth[IO](mockedUsers, mockedTokens, mockedEmails)
        result <- auth.sendPasswordRecoveryToken("someone@wrong.com")
    ...
    "sending recovery passwords should succeed for a user that exists" in {
      val program = for {
        ...
        result <- auth.sendPasswordRecoveryToken(romanEmail)
        usersBeingSentEmails <- set.get
      } yield usersBeingSentEmails

      program.asserting(_ should contain(romanEmail))
    }
  }
}
