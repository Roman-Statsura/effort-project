  ...
  "Jobs 'algebra'" - {
    ...
    "should create a new job" in {
      transactor.use { xa =>
        val program = for {
          jobs     <- LiveJobs[IO](xa)
          jobId    <- jobs.create("bla@mail.ru", SrsNewJob)
          maybeJob <- jobs.find(jobId)
        } yield maybeJob

        program.asserting(_.map(_.jobInfo) shouldBe Some(SrsNewJob))
      }
    }
    ...
    "should return zero updated rows if the job ID to deleted is not found" in {
      transactor.use { xa =>
        val program = for {
          jobs                <- LiveJobs[IO](xa)
